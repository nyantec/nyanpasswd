...
use axum::{extract::{State, Query, FromRequestParts}, response::IntoResponse, Form, http::request::Parts};
use chrono::{FixedOffset, DateTime};
use hyper::StatusCode;
use mail_passwd::{Password, User, axum::CertDn};
use sailfish::TemplateOnce;
use uuid::Uuid;

use crate::{Service, Layout, COMPANY_NAME, IMPRESSUM};
...
impl<T> FromRequestParts<T> for Admin where T: Send + Sync {
...
async fn deactivate_user(
	...
	match backend.toggle_user_login_allowed(form.uid).await {
		Ok(()) => (StatusCode::CREATED, [("Location", format!("/admin/manage_user?uid={}", form.uid))]).into_response(),
		Err(err) => (
			StatusCode::INTERNAL_SERVER_ERROR,
			[("Content-Type", "text/plain")],
			format!("SQL layer error: {}", err),
